...
func (s *Service) Prepare(ctx context.Context, req *api.PrepareRequest[CarReservationPayload]) (*api.PrepareResponse, error) {
	...
	startDate, err := time.Parse(config.DateFormat, req.Payload.CarStartDate)
	if err != nil {
		return &api.PrepareResponse{
			Success: false,
	...
	endDate, err := time.Parse(config.DateFormat, req.Payload.CarEndDate)
	if err != nil {
		return &api.PrepareResponse{
			Success: false,
	...
	if err := s.repo.PrepareCarReservation(
		ctx,
		req.TransactionID,
		req.Payload.CarID,
...
func (s *Service) Commit(ctx context.Context, req *api.CommitRequest) (*api.CommitResponse, error) {
	if err := s.repo.CommitCarReservation(ctx, req.TransactionID); err != nil {
...
func (s *Service) Abort(ctx context.Context, req *api.AbortRequest) (*api.AbortResponse, error) {
	if err := s.repo.AbortCarReservation(ctx, req.TransactionID); err != nil {
